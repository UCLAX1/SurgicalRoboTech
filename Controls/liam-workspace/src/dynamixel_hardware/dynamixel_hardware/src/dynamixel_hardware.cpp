  ...
  const rclcpp::Duration & /* period */)
{
  ...
  for (uint i = 0; i < ids.size(); i++) {
    ...
    if(i == 3){
      joints_[i].state.position = (dynamixel_workbench_.convertValue2Radian(ids[i], positions[i])) + radian_offset_3;
    }
    if(i == 4){
      joints_[i].state.position = (dynamixel_workbench_.convertValue2Radian(ids[i], positions[i])) + radian_offset_4;
    }
    if(i == 5){
      joints_[i].state.position = (dynamixel_workbench_.convertValue2Radian(ids[i], positions[i])) + radian_offset_5;
    }
...
CallbackReturn DynamixelHardware::set_joint_positions()
{
  ...
  for (uint i = 0; i < ids.size(); i++) {
    ...
    if(i == 3){
      commands[i] = dynamixel_workbench_.convertRadian2Value(ids[i], static_cast<float>(joints_[i].command.position - radian_offset_3));
    }
    if(i == 4){
      commands[i] = dynamixel_workbench_.convertRadian2Value(ids[i], static_cast<float>(joints_[i].command.position - radian_offset_4));
    }
    if(i == 5){
      commands[i] = dynamixel_workbench_.convertRadian2Value(ids[i], static_cast<float>(joints_[i].command.position - radian_offset_5));
    }   
  }
